...
#ifdef PSHELL_FRANKOS
    printf("\nPico Shell " PSHELL_GIT_TAG ", Vi " VI_VER "\n");
#else
...
int pshell_main(void) {
    ...
    printf("\nplatform: FRANK OS, console: window [%u X %u], filesystem: SD card\n\n",
           screen_x, screen_y);

    // In FRANK OS the FatFs filesystem is always available
    mounted = true;
